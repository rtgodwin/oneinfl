pred <- function(model, data, maxpred) {
  ...
  if(class(model) == "truncmodel" & model1$dist == "Poisson") {
    #PP
    pred <- rep(0, maxpred)
    for(pp in 1:(maxpred)){
    ...
    pred
  } else if(class(model) == "truncmodel" & model1$dist == "negbin") {
    #ZTNB
    pred <- rep(0, maxpred)
    for(pp in 1:(maxpred)){
    ...
    pred
  } else if(class(model) == "oneinflmodel" & model1$dist == "Poisson") {
    #OIPP
    pred <- rep(0, maxpred)
    ...
    pred
  } else if(class(model) == "oneinflmodel" & model1$dist == "negbin") {
    #OIZTNB
    pred <- rep(0, maxpred)
