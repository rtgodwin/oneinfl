dfee_pois <- function(b, g, X, Z, dummies) {
  ...
  # calculate the "marginal" effect for a dummy
  dummy.effect <- function(dummy) {
    if (any(colnames(X) == dummy) & any(colnames(Z) == dummy)) {
      Xd1 <- Xd0 <- X
      Xd1[, dummy] <- 1
      Xd0[, dummy] <- 0
      Zd1 <- Zd0 <- Z
      Zd1[, dummy] <- 1
      Zd0[, dummy] <- 0
      fee_pois(b, g, X=Xd1, Z=Zd1) - fee_pois(b, g, X=Xd0, Z=Zd0)
    } else if (any(colnames(X) == dummy)) {
      Xd1 <- Xd0 <- X
      Xd1[, dummy] <- 1
      Xd0[, dummy] <- 0
      fee_pois(b, g, X=Xd1, Z) - fee_pois(b, g, X=Xd0, Z)
    } else {
      Zd1 <- Zd0 <- Z
      Zd1[, dummy] <- 1
      Zd0[, dummy] <- 0
      fee_pois(b, g, X, Z=Zd1) - fee_pois(b, g, X, Z=Zd0)
    }
  }

  # change marginal FEE for dummies
  dfeedq[, dummies] <- sapply(dummies, dummy.effect)
  dfeedq
}
